...
;(function () {
  ...
  // --- React-crash guard ------------------------------------------------------
  // We wrap text nodes (math islands) inside a React-owned tree; when React
  // later reconciles a node we moved, removeChild/insertBefore throw and the
  // whole webview dies ("Something went wrong … Failed to execute 'removeChild'
  // on 'Node'"). Same crash Google Translate causes on React apps; same
  // well-known cure — soften exactly those two calls. This script runs in the
  // webview's own (main) world, so patching here reaches React.
  try {
